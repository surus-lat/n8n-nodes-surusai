...
export class SurusAi implements INodeType {
	description: INodeTypeDescription = {
		...
		description:
			'Translate text, transcribe audio, and extract structured data using SURUS AI services',
		defaults: {
			name: 'SURUS AI',
		},
		usableAsTool: true,
		...
		properties: [
			...
			// ----------------------------------
			{
				...
				default: true,
				description:
					'Whether to return only the extracted content instead of the full API response',
				displayOptions: {
					show: {
				...
				if (operation === 'translate') {
					...
					if (!binaryData) {
						throw new NodeOperationError(
							this.getNode(),
							'No binary data found for the audio file',
							{
								itemIndex: i,
							},
						);
					...
					const jsonSchema =
						typeof jsonSchemaRaw === 'string' ? JSON.parse(jsonSchemaRaw) : jsonSchemaRaw;
